...
void CWallet::ReacceptWalletTransactions() {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)&item, mapWallet)
    {
        ...
        if ((wtx.IsCoinBase() || wtx.IsCoinStake() || wtx.IsZerocoinSpend() || wtx.IsSigmaSpend()) && (nDepth == 0 && !wtx.isAbandoned()))
            continue;
        ...
        const CAmount amountToSpendLimit) const
{
    ...
    if (!MoneyRange(required)) {
        throw std::invalid_argument("Request to spend more than 21 MLN Noir.\n");
    }
...
//                continue;
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !(fNoirNode && pcoin->vout[i].nValue == NOIRNODE_COIN_REQUIRED * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fNoirNode) found = pcoin->vout[i].nValue != NOIRNODE_COIN_REQUIRED * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_1000) {

                    found = pcoin->vout[i].nValue == NOIRNODE_COIN_REQUIRED * COIN;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vecTxInRet, CAmount &nValueRet,
    ...
    BOOST_FOREACH(const COutput &out, vCoins)
    {
        ...
        if (fNoirNode && out.tx->vout[out.i].nValue == NOIRNODE_COIN_REQUIRED * COIN) continue; //noirnode input
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (fAnonymizable) {
                ...
                if (fNoirNode && wtx.vout[i].nValue == NOIRNODE_COIN_REQUIRED * COIN) continue;
            ...
            while (true) {
                ...
                if (!SelectCoins(vAvailableCoins, nValueToSelect, setCoins, nValueIn, coinControl)) {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 25000 NOR.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 25000 NOR.");
...
bool CWallet::CreateSigmaMintModel(
    ...
    for(const std::pair<sigma::CoinDenomination, int>& denominationPair: denominationPairs) {
        ...
        if (!DenominationToInteger(denomination, denominationValue)) {
            throw runtime_error(
                "mintzerocoin <amount>(0.1, 0.5, 1, 10, 100) (\"noiraddress\")\n");
        }
        ...
        if(coinCount < 0) {
            throw runtime_error("Coin count negative (\"noiraddress\")\n");
        }
...
bool CWallet::CreateZerocoinMintModelV2(
    ...
    BOOST_FOREACH(denominationPair, denominationPairs){
        ...
        switch(denominationValue){
            ...
            default:
                throw runtime_error(
                    "mintzerocoin <amount>(1,10,25,50,100) (\"noiraddress\")\n");
        }
...
bool CWallet::CreateZerocoinMintModel(string &stringError, const string& denomAmount, MintAlgorithm algo) {
    //TODO(martun) check if it is time to start minting sigma mints. Not sure how we can
...
//            LogPrintf("txNew:%s\n", txNew.ToString());
            ...
            bool useVersion2 = IsZerocoinTxV2(denomination, Params(), coinId);
